...
class [[eosio::contract]] tablec: public contract{
    ...
    public:
        ...
        print("[*] Parent Called\n");
       transaction out1{};
        out1.actions.emplace_back(permission_level{_self, "active"_n}, client , "main"_n, std::make_tuple(uint64_t(1), uint64_t(idx+11), uint64_t(randNum)));
	// out1.send();
	    out1.send(idx+11, _self, false);
